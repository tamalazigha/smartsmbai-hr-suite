...
Main branded dashboard. Run: streamlit run app_suite.py
6-agent pipeline for Certified Growth Agent hiring across 5 regions.
"""
...
with st.sidebar:
    ...
    st.page_link("pages/2_CV_Screening.py",      label="📄 Agent 2 — CV Screening (ATS)",  icon="2️⃣")
...
col6.metric("Offered",sc.get("offered",0)+sc.get("certified",0))
...
stages=[("📥 Ingested",["new"],"#D1FAE5","#065F46"),
        ...
        ("🎁 Offered",["offered","certified"],"#E0F2FE","#0369A1")]
kcols=st.columns(6)
...
awaiting=sc.get("interview_sent",0); pending_scoring=sc.get("scoring",0)+sc.get("interview_complete",0)
if awaiting>0:
    st.markdown("---"); st.info(f"📬 **{awaiting} candidate(s)** awaiting interview replies. Go to **Agent 4 — Response Scraper**.")
if pending_scoring>0:
    st.info(f"📊 **{pending_scoring} candidate(s)** ready for scoring. Go to **Agent 5 — Scoring**.")
...
if candidates:
    ...
    st.info("No applications yet. Use **Agent 1** to ingest your first applications from `info@smartsmbai.com`.")
    st.markdown("""| Step | Agent | Action |\n|---|---|---|\n|1|**Agent 1**|Fetch Growth Agent applications from inbox|
|2|**Agent 2**|ATS filter + Claude CV screening (region-aware)|
|3|**Agent 3**|Select advanced candidates → bulk invitations|
|4|**Agent 4**|Auto-scan inbox for replies → parse + score|
|5|**Agent 5**|Review AI scorecards → shortlist|
|6|**Agent 6**|Generate regional shortlist memo + certification gate|""")
